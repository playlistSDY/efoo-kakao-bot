...
사용자가 명확히 밝힌 알레르기, 평소 선호·비선호, 예산 상한, 기억할 식사 관련 메모는 사용자 프로필 DB에 요약해 저장합니다. 에이전트는 `save_user_memory` 도구로 기억을 추가·수정·삭제하며 저장된 프로필을 이후 모든 AI 대화의 문맥에 포함합니다. 모델을 생략하는 빠른 메뉴 조회도 저장된 설정을 별도 구획에 표시합니다. 일회성 요구나 추측한 취향은 장기 기억으로 저장하지 않습니다.
...
사용자가 명확히 말한 장기적인 식사 관련 선호, 비선호, 알레르기, 예산과 메모를 프로필 DB에 저장하거나 정정·삭제합니다. 저장된 내용은 이후 추천과 비교 요청에서 항상 참고합니다.
